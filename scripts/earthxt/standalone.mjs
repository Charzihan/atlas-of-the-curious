...
import { root, runtimeGraph, moduleImports, moduleResources, declarationExports,
  assertNoModuleSyntax, stylesheetLinks, scriptLinks, cssImports, resolveReference, resolveCSSReference } from './graph.mjs';

function bundleModules(graph, entry, entries = [entry]) {
  ...
  for (const filename of entries) visit(filename);
  const script = `(() => {\n'use strict';\nconst modules = Object.create(null);\n${blocks.join('\n')}\nreturn modules[${JSON.stringify(entry)}];\n})();`;
  new Script(script, { filename: 'earthxt-standalone.js' });
  return script;
}

// Classic scripts run in document order before deferred module entry points.
// Keep a single module registry even if a page gains another module entry.
export function bundlePageScripts(graph, template, file = 'earthxt/index.html') {
  const scripts = scriptLinks(template), classics = [], modules = [];
  for (const script of scripts) {
    const filename = resolveReference(file, script.reference);
    if (script.type === 'module') modules.push(filename);
    else if (['classic', 'text/javascript', 'application/javascript'].includes(script.type)) {
      const source = graph.get(filename).bytes.toString('utf8');
      assertNoModuleSyntax(source, filename);
      classics.push(source);
    } else throw new Error(`Unsupported script type: ${script.type}`);
  }
  const source = [...classics, modules.length ? bundleModules(graph, modules.at(-1), modules) : ''].join('\n;\n');
  new Script(source);
  return source;
}
...
export async function buildStandalone() {
  ...
  const bundled = bundlePageScripts(graph, template);
  const style = `\n${css}\n`;
  ...
  for (const script of scriptLinks(template)) html = html.replace(script.statement, '');
  html = html.replace('<html lang="en">', '<html lang="en" data-standalone>')
