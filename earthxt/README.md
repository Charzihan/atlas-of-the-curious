...
Place routing uses the renderer’s current grid: country cells and cells whose corners fall outside the sphere’s disc are blocked, as are country-pill rectangles and a three-by-three-cell footprint around each visible marker. The shared label engine reserves enough rows for the `map-label` role’s leading and returns the full occupied cells. It warms all names before animation; frames never call pretext preparation. A larger anchor search lets inland places reach the coast. Once routed, placements are held until the camera moves at least one cell, the visible set changes, or the grid/level changes. Held placements are still checked against current land and marker cells on every frame; an invalid label is omitted until the next placement. Labels paint in uppercase with registry tracking exactly once. Moving this routing into `js/text-worker.js` is a follow-up.
...
The footer shows rendered FPS, visible glyphs, altitude, and resident geographic bytes. Diagnostics add candidate screen cells, current LOD, camera distance, projection plus geographic lookup time, text plus label submission time, current/average name layout and drawing time, silhouette/pill counts, visible/labelled place counts, place routing/drawing time, and JS heap usage when the browser exposes it. Timings are CPU submission measurements; they do not measure compositor/GPU completion. FPS counts actual rendered frames, so a paused view can correctly show 0–2 FPS while it redraws only for occasional ocean changes. Metrics update every 600 ms. `EARTHXT_DEBUG.snapshot()` exposes `placeCount`, `visibleMarkers`, `labelledPlaces`, `labelCells`, `markerCells`, `markers`, `placeLabels`, and `placeLayoutPasses`. With `{ includeGeometry: true }`, the grid also includes current `countryIds` and `blockedCells`. Snapshots copy these records only on explicit inspection.
...
The place browser checks open South America at Coastlines detail, require at least four visible and labelled places, check all reported cells against current water/disc/marker masks, exercise hover/taglines, card bounds, Tab and Enter, click destinations, and place deep links in both directions. A two-second rotation sample writes `test-results/earthxt/places-timing.json` and enforces p95 CPU submission below 33.4 ms and p95 frame intervals below 50 ms at the suite’s desktop viewport. This is a regression budget for that test environment, not a device-independent guarantee. The offline check opens a place deep link in the standalone file with HTTP blocked and verifies all 40 places, the story excerpt, and marker activation without navigation.
